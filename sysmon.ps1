...
# Динамическое определение реальных имён комплектующих через WMI
    ...
    while ($true) {
        ...
        try {
            ...
            if ($json) {
                ...
                # Отбираем узлы с численной температурой
        ...
        Write-Host "CPU Temp:   $cpuTemp ($cpuName)" -ForegroundColor Green
        Write-Host "--------------------------------------------------" -ForegroundColor DarkGray

        Write-Host ("GPU Load:   {0,3}% " -f $gpuLoad) -NoNewline
        Write-Host (Get-ProgressBar $gpuLoad) -ForegroundColor Yellow
        Write-Host "GPU Temp:   $gpuTemp ($gpuName)" -ForegroundColor Cyan
        Write-Host "--------------------------------------------------" -ForegroundColor DarkGray
